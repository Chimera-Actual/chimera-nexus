...
import { TaskScheduler } from "../../../src/core/scheduler/task-scheduler";
import { ScheduledTask, PermissionMode } from "../../../src/core/types";

// ---------------------------------------------------------------------------
// Mock Vault factory
// Avoid instantiating TFile/TFolder directly inside the factory to prevent
// the circular-constructor stack overflow (TFile → TAbstractFile → Vault →
// TFile → ...) that occurs when using the mock class hierarchy at module load.
// ---------------------------------------------------------------------------
...
/** Plain-object stub that passes instanceof TFile check via the mock class. */
function makeTFileLike(path: string) {
  // Import lazily here to avoid the circular init at module parse time.
...
describe("TaskScheduler", () => {
  ...
  beforeEach(() => {
    // Clear first so that factory mock implementations are not wiped
    jest.clearAllMocks();
  ...
  // The source code uses `await import("obsidian")` for TFile, producing a
  // different class instance from `jest.requireMock("obsidian")`.  Using
  // Object.create with the *dynamically-imported* TFile prototype (without
  // calling the constructor) bypasses the circular-constructor stack overflow
  // while still passing the `instanceof TFile` guard in the source.
