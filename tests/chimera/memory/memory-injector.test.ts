/**
 * @file Tests for MemoryInjector (chimera memory module)
 */

import { Vault } from "obsidian";
import { MemoryInjector } from "../../../src/chimera/memory/memory-injector";
import {
  MemoryFile,
  MemoryTier,
  ChimeraMemorySettings,
  PermissionMode,
} from "../../../src/chimera/types";
...
function makeSettings(overrides: Partial<ChimeraMemorySettings> = {}): ChimeraMemorySettings {
  return {
    memoryEnabled: true,
...
describe("MemoryInjector", () => {
  ...
  // 7. buildMemoryContext assembles memory layers
  it("buildMemoryContext includes base system prompt", async () => {
    // No CLAUDE.md, no memory files
    (vault.adapter.exists as jest.Mock).mockResolvedValue(false);
    (vault.adapter.list as jest.Mock).mockResolvedValue({ files: [], folders: [] });

    const ctx = await injector.buildMemoryContext();

    expect(ctx).toContain("You are Chimera Nexus");
  });

  it("buildMemoryContext includes CLAUDE.md content when present", async () => {
  ...
  // 8. buildSystemPromptContext is an alias for buildMemoryContext
  it("buildSystemPromptContext is an alias for buildMemoryContext", async () => {
    (vault.adapter.exists as jest.Mock).mockResolvedValue(false);
    (vault.adapter.list as jest.Mock).mockResolvedValue({ files: [], folders: [] });
  ...
  // 9. buildPermissionInstruction returns correct text for each mode
  it("buildPermissionInstruction returns ask text for AskBeforeEdits", () => {
    const result = injector.buildPermissionInstruction(PermissionMode.AskBeforeEdits);
    expect(result).toBe("Ask for approval before making each edit.");
  });

  it("buildPermissionInstruction returns pre-approved text for BypassPermissions", () => {
    const result = injector.buildPermissionInstruction(PermissionMode.BypassPermissions);
    expect(result).toBe("All operations are pre-approved.");
  });

  it("buildPermissionInstruction returns plan text for Plan mode", () => {
    const result = injector.buildPermissionInstruction(PermissionMode.Plan);
    expect(result).toBe("Explore the code and present a plan before editing.");
  });

  it("buildPermissionInstruction returns auto-edit text for EditAutomatically", () => {
    const result = injector.buildPermissionInstruction(PermissionMode.EditAutomatically);
    expect(result).toBe("You may edit files automatically without asking.");
  });
});
