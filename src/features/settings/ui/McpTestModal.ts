...
export class McpTestModal extends Modal {
  ...
  onOpen() {
    ...
    this.modalEl.addClass('chimera-mcp-test-modal');
  ...
  private renderLoading() {
    ...
    const loadingEl = this.contentEl_.createDiv({ cls: 'chimera-mcp-test-loading' });

    const spinnerEl = loadingEl.createDiv({ cls: 'chimera-mcp-test-spinner' });
  ...
  private render() {
    ...
    const statusEl = this.contentEl_.createDiv({ cls: 'chimera-mcp-test-status' });

    const iconEl = statusEl.createSpan({ cls: 'chimera-mcp-test-icon' });
    ...
    const textEl = statusEl.createSpan({ cls: 'chimera-mcp-test-text' });
    if (this.result.success) {
    ...
    if (this.result.error) {
      const errorEl = this.contentEl_.createDiv({ cls: 'chimera-mcp-test-error' });
    ...
    if (this.result.tools.length > 0) {
      const toolsSection = this.contentEl_.createDiv({ cls: 'chimera-mcp-test-tools' });

      const toolsHeader = toolsSection.createDiv({ cls: 'chimera-mcp-test-tools-header' });
      toolsHeader.setText(`Available Tools (${this.result.tools.length})`);

      const toolsList = toolsSection.createDiv({ cls: 'chimera-mcp-test-tools-list' });

      for (const tool of this.result.tools) {
        this.renderTool(toolsList, tool);
      }
    } else if (this.result.success) {
      const noToolsEl = this.contentEl_.createDiv({ cls: 'chimera-mcp-test-no-tools' });
      noToolsEl.setText('No tools information available. Tools will be loaded when used in chat.');
    }

    const buttonContainer = this.contentEl_.createDiv({ cls: 'chimera-mcp-test-buttons' });

    if (this.result.tools.length > 0 && this.onToolToggle) {
      this.toggleAllBtn = buttonContainer.createEl('button', {
        cls: 'chimera-mcp-toggle-all-btn',
      });
  ...
  private renderTool(container: HTMLElement, tool: McpTool) {
    const toolEl = container.createDiv({ cls: 'chimera-mcp-test-tool' });

    const headerEl = toolEl.createDiv({ cls: 'chimera-mcp-test-tool-header' });

    const iconEl = headerEl.createSpan({ cls: 'chimera-mcp-test-tool-icon' });
    setIcon(iconEl, 'wrench');

    const nameEl = headerEl.createSpan({ cls: 'chimera-mcp-test-tool-name' });
    nameEl.setText(tool.name);

    const toggleEl = headerEl.createDiv({ cls: 'chimera-mcp-test-tool-toggle' });
    ...
    if (tool.description) {
      const descEl = toolEl.createDiv({ cls: 'chimera-mcp-test-tool-desc' });
  ...
  private updateToolState(toolEl: HTMLElement, enabled: boolean) {
    toolEl.toggleClass('chimera-mcp-test-tool-disabled', !enabled);
  }
