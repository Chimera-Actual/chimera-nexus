...
export class EnvSnippetModal extends Modal {
  ...
  onOpen() {
    ...
    this.modalEl.addClass('chimera-env-snippet-modal');
    ...
    const renderContextLimitFields = () => {
      ...
      for (const modelId of uniqueModelIds) {
        const row = contextLimitsContainer.createDiv({ cls: 'chimera-snippet-limit-row' });
        row.createSpan({ text: modelId, cls: 'chimera-snippet-limit-model' });
        row.createSpan({ cls: 'chimera-snippet-limit-spacer' });

        const input = row.createEl('input', {
          type: 'text',
          placeholder: '200k',
          cls: 'chimera-snippet-limit-input',
        });
    ...
    envVarsSetting.settingEl.addClass('chimera-env-snippet-setting');
    envVarsSetting.controlEl.addClass('chimera-env-snippet-control');

    contextLimitsContainer = contentEl.createDiv({ cls: 'chimera-snippet-context-limits' });
    renderContextLimitFields();

    const buttonContainer = contentEl.createDiv({ cls: 'chimera-snippet-buttons' });
...
export class EnvSnippetManager {
  ...
  private render() {
    ...
    const headerEl = this.containerEl.createDiv({ cls: 'chimera-snippet-header' });
    headerEl.createSpan({ text: t('settings.envSnippets.name'), cls: 'chimera-snippet-label' });
    ...
    if (snippets.length === 0) {
      const emptyEl = this.containerEl.createDiv({ cls: 'chimera-snippet-empty' });
      emptyEl.setText(t('settings.envSnippets.noSnippets'));
      return;
    }

    const listEl = this.containerEl.createDiv({ cls: 'chimera-snippet-list' });

    for (const snippet of snippets) {
      const itemEl = listEl.createDiv({ cls: 'chimera-snippet-item' });

      const infoEl = itemEl.createDiv({ cls: 'chimera-snippet-info' });

      const nameEl = infoEl.createDiv({ cls: 'chimera-snippet-name' });
      nameEl.setText(snippet.name);

      if (snippet.description) {
        const descEl = infoEl.createDiv({ cls: 'chimera-snippet-description' });
        descEl.setText(snippet.description);
      }

      const actionsEl = itemEl.createDiv({ cls: 'chimera-snippet-actions' });
      ...
      const deleteBtn = actionsEl.createEl('button', {
        cls: 'chimera-settings-action-btn chimera-settings-delete-btn',
  ...
  private async insertSnippet(snippet: EnvSnippet) {
    ...
    const envTextarea = document.querySelector('.chimera-settings-env-textarea') as HTMLTextAreaElement;
    if (envTextarea) {
    ...
    const view = this.plugin.app.workspace.getLeavesOfType('chimera-nexus-view')[0]?.view as ClaudianView | undefined;
