...
export class StatusPanel {
  ...
  private createPanel(): void {
    ...
    this.panelEl.className = 'chimera-status-panel';

    // Bash output container - hidden by default
    this.bashOutputContainerEl = document.createElement('div');
    this.bashOutputContainerEl.className = 'chimera-status-panel-bash';
    this.bashOutputContainerEl.style.display = 'none';

    this.bashHeaderEl = document.createElement('div');
    this.bashHeaderEl.className = 'chimera-tool-header chimera-status-panel-bash-header';
    ...
    this.bashContentEl.className = 'chimera-status-panel-bash-content';
    ...
    this.todoContainerEl.className = 'chimera-status-panel-todos';
    ...
    this.todoHeaderEl.className = 'chimera-status-panel-header';
    ...
    this.todoContentEl.className = 'chimera-status-panel-content chimera-todo-list-container';
  ...
  private renderTodoHeader(completedCount: number, totalCount: number, currentTask: TodoItem | undefined): void {
    ...
    icon.className = 'chimera-status-panel-icon';
    setIcon(icon, getToolIcon(TOOL_TODO_WRITE));
    this.todoHeaderEl.appendChild(icon);

    // Label
    const label = document.createElement('span');
    label.className = 'chimera-status-panel-label';
    ...
    if (!this.isTodoExpanded) {
      ...
      if (completedCount === totalCount && totalCount > 0) {
        ...
        status.className = 'chimera-status-panel-status status-completed';
      ...
      if (currentTask) {
        ...
        current.className = 'chimera-status-panel-current';
  ...
  private renderBashOutputs(options: { scroll?: boolean } = {}): void {
    ...
    headerIconEl.className = 'chimera-tool-icon';
    ...
    headerLabelEl.className = 'chimera-tool-label';
    ...
    previewEl.className = 'chimera-tool-current';
    ...
    summaryStatusEl.className = 'chimera-tool-status';
    ...
    actionsEl.className = 'chimera-status-panel-bash-actions';
  ...
  private renderBashEntry(info: PanelBashOutput): HTMLElement {
    ...
    entryLabelEl.className = 'chimera-tool-label';
    ...
    entryStatusEl.className = 'chimera-tool-status';
    ...
    contentEl.className = 'chimera-tool-content';
    ...
    rowEl.className = 'chimera-tool-result-row';

    const textEl = document.createElement('span');
    textEl.className = 'chimera-tool-result-text';
  ...
  private appendActionButton(
    ...
    el.className = `chimera-status-panel-bash-action chimera-status-panel-bash-action-${name}`;
