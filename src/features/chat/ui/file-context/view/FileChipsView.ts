...
export class FileChipsView {
  ...
  constructor(containerEl: HTMLElement, callbacks: FileChipsViewCallbacks) {
    ...
    this.fileIndicatorEl = this.containerEl.createDiv({ cls: 'chimera-file-indicator' });
    if (firstChild) {
  ...
  private renderFileChip(filePath: string, onRemove: () => void): void {
    const chipEl = this.fileIndicatorEl.createDiv({ cls: 'chimera-file-chip' });

    const iconEl = chipEl.createSpan({ cls: 'chimera-file-chip-icon' });
    setIcon(iconEl, 'file-text');

    const normalizedPath = filePath.replace(/\\/g, '/');
    const filename = normalizedPath.split('/').pop() || filePath;
    const nameEl = chipEl.createSpan({ cls: 'chimera-file-chip-name' });
    nameEl.setText(filename);
    nameEl.setAttribute('title', filePath);

    const removeEl = chipEl.createSpan({ cls: 'chimera-file-chip-remove' });
    removeEl.setText('\u00D7');
    removeEl.setAttribute('aria-label', 'Remove');

    chipEl.addEventListener('click', (e) => {
      if (!(e.target as HTMLElement).closest('.chimera-file-chip-remove')) {
