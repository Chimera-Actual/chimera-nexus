...
export class ImageContextManager {
  ...
  constructor(
    ...
    const fileIndicator = this.previewContainerEl.querySelector('.chimera-file-indicator');
    this.imagePreviewEl = this.previewContainerEl.createDiv({ cls: 'chimera-image-preview' });
  ...
  private setupDragAndDrop() {
    const inputWrapper = this.containerEl.querySelector('.chimera-input-wrapper') as HTMLElement;
    if (!inputWrapper) return;

    this.dropOverlay = inputWrapper.createDiv({ cls: 'chimera-drop-overlay' });
    const dropContent = this.dropOverlay.createDiv({ cls: 'chimera-drop-content' });
  ...
  private handleDragLeave(e: DragEvent) {
    ...
    const inputWrapper = this.containerEl.querySelector('.chimera-input-wrapper');
    if (!inputWrapper) {
  ...
  private renderImagePreview(id: string, image: ImageAttachment) {
    const previewEl = this.imagePreviewEl.createDiv({ cls: 'chimera-image-chip' });

    const thumbEl = previewEl.createDiv({ cls: 'chimera-image-thumb' });
    ...
    const infoEl = previewEl.createDiv({ cls: 'chimera-image-info' });
    const nameEl = infoEl.createSpan({ cls: 'chimera-image-name' });
    nameEl.setText(this.truncateName(image.name, 20));
    nameEl.setAttribute('title', image.name);

    const sizeEl = infoEl.createSpan({ cls: 'chimera-image-size' });
    sizeEl.setText(this.formatSize(image.size));

    const removeEl = previewEl.createSpan({ cls: 'chimera-image-remove' });
    removeEl.setText('\u00D7');
