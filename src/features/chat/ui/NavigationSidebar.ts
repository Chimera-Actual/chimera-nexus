...
export class NavigationSidebar {
  ...
  constructor(
    ...
    this.container = this.parentEl.createDiv({ cls: 'chimera-nav-sidebar' });

    // Create buttons
    this.topBtn = this.createButton('chimera-nav-btn-top', 'chevrons-up', 'Scroll to top');
    this.prevBtn = this.createButton('chimera-nav-btn-prev', 'chevron-up', 'Previous message');
    this.nextBtn = this.createButton('chimera-nav-btn-next', 'chevron-down', 'Next message');
    this.bottomBtn = this.createButton('chimera-nav-btn-bottom', 'chevrons-down', 'Scroll to bottom');
  ...
  private createButton(cls: string, icon: string, label: string): HTMLElement {
    const btn = this.container.createDiv({ cls: `chimera-nav-btn ${cls}` });
    setIcon(btn, icon);
  ...
  private scrollToMessage(direction: 'prev' | 'next'): void {
    const messages = Array.from(this.messagesEl.querySelectorAll('.chimera-message-user')) as HTMLElement[];
