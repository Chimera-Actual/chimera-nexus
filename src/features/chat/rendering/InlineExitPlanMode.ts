...
export class InlineExitPlanMode {
  ...
  render(): void {
    this.rootEl = this.containerEl.createDiv({ cls: 'chimera-plan-approval-inline' });

    const titleEl = this.rootEl.createDiv({ cls: 'chimera-plan-inline-title' });
    titleEl.setText('Plan complete');

    this.planContent = this.readPlanContent();
    if (this.planContent) {
      const contentEl = this.rootEl.createDiv({ cls: 'chimera-plan-content-preview' });
      if (this.renderContent) {
        void this.renderContent(contentEl, this.planContent);
      } else {
        contentEl.createDiv({ cls: 'chimera-plan-content-text', text: this.planContent });
      }
    } else if (this.planReadError) {
      this.rootEl.createDiv({
        cls: 'chimera-plan-content-preview chimera-plan-read-error',
    ...
    if (allowedPrompts && Array.isArray(allowedPrompts) && allowedPrompts.length > 0) {
      const permEl = this.rootEl.createDiv({ cls: 'chimera-plan-permissions' });
      permEl.createDiv({ text: 'Requested permissions:', cls: 'chimera-plan-permissions-label' });
      const listEl = permEl.createEl('ul', { cls: 'chimera-plan-permissions-list' });
      for (const perm of allowedPrompts) {
        listEl.createEl('li', { text: perm.prompt });
      }
    }

    const actionsEl = this.rootEl.createDiv({ cls: 'chimera-ask-list' });

    const newSessionRow = actionsEl.createDiv({ cls: 'chimera-ask-item' });
    newSessionRow.addClass('is-focused');
    newSessionRow.createSpan({ text: '\u203A', cls: 'chimera-ask-cursor' });
    newSessionRow.createSpan({ text: '1. ', cls: 'chimera-ask-item-num' });
    newSessionRow.createSpan({ text: 'Approve (new session)', cls: 'chimera-ask-item-label' });
    ...
    const approveRow = actionsEl.createDiv({ cls: 'chimera-ask-item' });
    approveRow.createSpan({ text: '\u00A0', cls: 'chimera-ask-cursor' });
    approveRow.createSpan({ text: '2. ', cls: 'chimera-ask-item-num' });
    approveRow.createSpan({ text: 'Approve (current session)', cls: 'chimera-ask-item-label' });
    ...
    const feedbackRow = actionsEl.createDiv({ cls: 'chimera-ask-item chimera-ask-custom-item' });
    feedbackRow.createSpan({ text: '\u00A0', cls: 'chimera-ask-cursor' });
    feedbackRow.createSpan({ text: '3. ', cls: 'chimera-ask-item-num' });
    ...
    this.rootEl.createDiv({ text: HINTS_TEXT, cls: 'chimera-ask-hints' });
