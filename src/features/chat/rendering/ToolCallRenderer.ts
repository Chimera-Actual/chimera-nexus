...
function renderWebSearchExpanded(container: HTMLElement, result: string): void {
  ...
  const linksEl = container.createDiv({ cls: 'chimera-tool-lines' });
  for (const link of parsed.links) {
    const linkEl = linksEl.createEl('a', { cls: 'chimera-tool-link' });
    linkEl.setAttribute('href', link.url);
    linkEl.setAttribute('target', '_blank');
    linkEl.setAttribute('rel', 'noopener noreferrer');

    const iconEl = linkEl.createSpan({ cls: 'chimera-tool-link-icon' });
    setIcon(iconEl, 'external-link');

    linkEl.createSpan({ cls: 'chimera-tool-link-title', text: link.title });
  }

  if (parsed.summary) {
    const summaryEl = container.createDiv({ cls: 'chimera-tool-web-summary' });
...
function renderFileSearchExpanded(container: HTMLElement, result: string): void {
  ...
  if (lines.length === 0) {
    container.createDiv({ cls: 'chimera-tool-empty', text: 'No matches found' });
    return;
  }
...
function renderLinesExpanded(
  ...
  const linesEl = container.createDiv({ cls: 'chimera-tool-lines' });
  for (const line of displayLines) {
    const stripped = line.replace(/^\s*\d+→/, '');
    const lineEl = linesEl.createDiv({ cls: 'chimera-tool-line' });
...
function renderToolSearchExpanded(container: HTMLElement, result: string): void {
  ...
  for (const name of toolNames) {
    const lineEl = container.createDiv({ cls: 'chimera-tool-search-item' });
    const iconEl = lineEl.createSpan({ cls: 'chimera-tool-search-icon' });
...
export function renderExpandedContent(container: HTMLElement, toolName: string, result: string | undefined): void {
  if (!result) {
    container.createDiv({ cls: 'chimera-tool-empty', text: 'No result' });
    return;
  }
...
function resetStatusElement(statusEl: HTMLElement, statusClass: string, ariaLabel: string): void {
  statusEl.className = 'chimera-tool-status';
  statusEl.empty();
...
export function renderTodoWriteResult(
  ...
  container.empty();
  container.addClass('chimera-todo-panel-content');
  container.addClass('chimera-todo-list-container');

  const todos = input.todos as TodoItem[] | undefined;
  if (!todos || !Array.isArray(todos)) {
    const item = container.createSpan({ cls: 'chimera-tool-result-item' });
...
function createToolElementStructure(
  ...
  const toolEl = parentEl.createDiv({ cls: 'chimera-tool-call' });

  const header = toolEl.createDiv({ cls: 'chimera-tool-header' });
  ...
  const nameEl = header.createSpan({ cls: 'chimera-tool-name' });
  nameEl.setText(getToolName(toolCall.name, toolCall.input));

  const summaryEl = header.createSpan({ cls: 'chimera-tool-summary' });
  ...
  const statusEl = header.createSpan({ cls: 'chimera-tool-status' });

  const content = toolEl.createDiv({ cls: 'chimera-tool-content' });
...
function renderAskUserQuestionResult(container: HTMLElement, toolCall: ToolCallInfo): boolean {
  ...
  const reviewEl = container.createDiv({ cls: 'chimera-ask-review' });
  for (let i = 0; i < questions.length; i++) {
    const q = questions[i];
    const answer = formatAnswer(answers[q.question]);
    const pairEl = reviewEl.createDiv({ cls: 'chimera-ask-review-pair' });
    pairEl.createDiv({ text: `${i + 1}.`, cls: 'chimera-ask-review-num' });
    ...
    bodyEl.createDiv({
      ...
      cls: answer ? 'chimera-ask-review-a-text' : 'chimera-ask-review-empty',
    });
  }
...
function contentFallback(container: HTMLElement, text: string): void {
  const resultRow = container.createDiv({ cls: 'chimera-tool-result-row' });
  const resultText = resultRow.createSpan({ cls: 'chimera-tool-result-text' });
...
function createCurrentTaskPreview(
  ...
  const currentTaskEl = header.createSpan({ cls: 'chimera-tool-current' });
...
export function updateToolCallResult(
  ...
  if (toolCall.name === TOOL_TODO_WRITE) {
    const statusEl = toolEl.querySelector('.chimera-tool-status') as HTMLElement;
    if (statusEl) {
      setTodoWriteStatus(statusEl, toolCall.input);
    }
    const content = toolEl.querySelector('.chimera-tool-content') as HTMLElement;
    if (content) {
      renderTodoWriteResult(content, toolCall.input);
    }
    const nameEl = toolEl.querySelector('.chimera-tool-name') as HTMLElement;
    if (nameEl) {
      nameEl.setText(getToolName(toolCall.name, toolCall.input));
    }
    const currentTaskEl = toolEl.querySelector('.chimera-tool-current') as HTMLElement;
    if (currentTaskEl) {
