...
export function createThinkingBlock(
  ...
  const wrapperEl = parentEl.createDiv({ cls: 'chimera-thinking-block' });

  // Header (clickable to expand/collapse)
  const header = wrapperEl.createDiv({ cls: 'chimera-thinking-header' });
  ...
  // Label with timer
  const labelEl = header.createSpan({ cls: 'chimera-thinking-label' });
  ...
  const contentEl = wrapperEl.createDiv({ cls: 'chimera-thinking-content' });
...
export function finalizeThinkingBlock(state: ThinkingBlockState): number {
  ...
  const header = state.wrapperEl.querySelector('.chimera-thinking-header');
  if (header) {
