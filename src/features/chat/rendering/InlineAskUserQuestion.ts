...
export class InlineAskUserQuestion {
  ...
  render(): void {
    this.rootEl = this.containerEl.createDiv({ cls: 'chimera-ask-question-inline' });

    const titleEl = this.rootEl.createDiv({ cls: 'chimera-ask-inline-title' });
    ...
    if (!this.config.immediateSelect) {
      this.tabBar = this.rootEl.createDiv({ cls: 'chimera-ask-tab-bar' });
      this.renderTabBar();
    }
    this.contentArea = this.rootEl.createDiv({ cls: 'chimera-ask-content' });
  ...
  private renderTabBar(): void {
    ...
    for (let idx = 0; idx < this.questions.length; idx++) {
      ...
      const tab = this.tabBar.createSpan({ cls: 'chimera-ask-tab' });
      tab.createSpan({ text: this.questions[idx].header, cls: 'chimera-ask-tab-label' });
      tab.createSpan({ text: answered ? ' \u2713' : '', cls: 'chimera-ask-tab-tick' });
    ...
    const submitTab = this.tabBar.createSpan({ cls: 'chimera-ask-tab' });
    submitTab.createSpan({ text: allAnswered ? '\u2713 ' : '', cls: 'chimera-ask-tab-submit-check' });
    submitTab.createSpan({ text: 'Submit', cls: 'chimera-ask-tab-label' });
  ...
  private renderQuestionTab(idx: number): void {
    ...
    this.contentArea.createDiv({
      text: q.question,
      cls: 'chimera-ask-question-text',
    });

    const listEl = this.contentArea.createDiv({ cls: 'chimera-ask-list' });
    ...
    for (let optIdx = 0; optIdx < q.options.length; optIdx++) {
      ...
      const row = listEl.createDiv({ cls: 'chimera-ask-item' });
      if (isFocused) row.addClass('is-focused');
      if (isSelected) row.addClass('is-selected');

      row.createSpan({ text: isFocused ? '\u203A' : '\u00A0', cls: 'chimera-ask-cursor' });
      row.createSpan({ text: `${optIdx + 1}. `, cls: 'chimera-ask-item-num' });

      if (isMulti) {
        this.renderMultiSelectCheckbox(row, isSelected);
      }

      const labelBlock = row.createDiv({ cls: 'chimera-ask-item-content' });
      const labelRow = labelBlock.createDiv({ cls: 'chimera-ask-label-row' });
      labelRow.createSpan({ text: option.label, cls: 'chimera-ask-item-label' });

      if (!isMulti && isSelected) {
        labelRow.createSpan({ text: ' \u2713', cls: 'chimera-ask-check-mark' });
      }

      if (option.description) {
        labelBlock.createDiv({ text: option.description, cls: 'chimera-ask-item-desc' });
      }
    ...
    if (this.config.showCustomInput) {
      ...
      const customRow = listEl.createDiv({ cls: 'chimera-ask-item chimera-ask-custom-item' });
      if (customFocused) customRow.addClass('is-focused');

      customRow.createSpan({ text: customFocused ? '\u203A' : '\u00A0', cls: 'chimera-ask-cursor' });
      customRow.createSpan({ text: `${customIdx + 1}. `, cls: 'chimera-ask-item-num' });
  ...
  private renderSubmitTab(): void {
    this.contentArea.createDiv({
      ...
      cls: 'chimera-ask-review-title',
    });

    const reviewEl = this.contentArea.createDiv({ cls: 'chimera-ask-review' });

    for (let idx = 0; idx < this.questions.length; idx++) {
      const q = this.questions[idx];
      const answerText = this.getAnswerText(idx);

      const pairEl = reviewEl.createDiv({ cls: 'chimera-ask-review-pair' });
      pairEl.createDiv({ text: `${idx + 1}.`, cls: 'chimera-ask-review-num' });
      const bodyEl = pairEl.createDiv({ cls: 'chimera-ask-review-body' });
      bodyEl.createDiv({ text: q.question, cls: 'chimera-ask-review-q-text' });
      bodyEl.createDiv({
        text: answerText || 'Not answered',
        cls: answerText ? 'chimera-ask-review-a-text' : 'chimera-ask-review-empty',
      });
    ...
    this.contentArea.createDiv({
      ...
      cls: 'chimera-ask-review-prompt',
    });

    const actionsEl = this.contentArea.createDiv({ cls: 'chimera-ask-list' });
    const allAnswered = this.questions.every((_, i) => this.isQuestionAnswered(i));

    const submitRow = actionsEl.createDiv({ cls: 'chimera-ask-item' });
    if (this.focusedItemIndex === 0) submitRow.addClass('is-focused');
    if (!allAnswered) submitRow.addClass('is-disabled');
    submitRow.createSpan({ text: this.focusedItemIndex === 0 ? '\u203A' : '\u00A0', cls: 'chimera-ask-cursor' });
    submitRow.createSpan({ text: '1. ', cls: 'chimera-ask-item-num' });
    submitRow.createSpan({ text: 'Submit answers', cls: 'chimera-ask-item-label' });
    ...
    const cancelRow = actionsEl.createDiv({ cls: 'chimera-ask-item' });
    if (this.focusedItemIndex === 1) cancelRow.addClass('is-focused');
    cancelRow.createSpan({ text: this.focusedItemIndex === 1 ? '\u203A' : '\u00A0', cls: 'chimera-ask-cursor' });
    cancelRow.createSpan({ text: '2. ', cls: 'chimera-ask-item-num' });
    cancelRow.createSpan({ text: 'Cancel', cls: 'chimera-ask-item-label' });
  ...
  private renderMultiSelectCheckbox(parent: HTMLElement, checked: boolean): void {
    parent.createSpan({
      text: checked ? '[\u2713] ' : '[ ] ',
      cls: `chimera-ask-check${checked ? ' is-checked' : ''}`,
    });
  }
  ...
  private updateOptionVisuals(qIdx: number): void {
    ...
    for (let i = 0; i < q.options.length; i++) {
      ...
      if (isMulti) {
        const checkSpan = item.querySelector('.chimera-ask-check') as HTMLElement | null;
        if (checkSpan) {
          checkSpan.textContent = isSelected ? '[\u2713] ' : '[ ] ';
          checkSpan.toggleClass('is-checked', isSelected);
        }
      } else {
        const labelRow = item.querySelector('.chimera-ask-label-row') as HTMLElement | null;
        const existingMark = item.querySelector('.chimera-ask-check-mark');
  ...
  private updateFocusIndicator(): void {
    for (let i = 0; i < this.currentItems.length; i++) {
      const item = this.currentItems[i];
      const cursor = item.querySelector('.chimera-ask-cursor');
      if (i === this.focusedItemIndex) {
        item.addClass('is-focused');
        if (cursor) cursor.textContent = '\u203A';
        item.scrollIntoView({ block: 'nearest' });

        if (item.hasClass('chimera-ask-custom-item')) {
          const input = item.querySelector('.chimera-ask-custom-text') as HTMLInputElement;
          if (input) {
            input.focus();
            this.isInputFocused = true;
          }
  ...
  private updateTabIndicators(): void {
    for (let idx = 0; idx < this.questions.length; idx++) {
      ...
      const tick = tab.querySelector('.chimera-ask-tab-tick');
      const answered = this.isQuestionAnswered(idx);
    ...
    if (submitTab) {
      const submitCheck = submitTab.querySelector('.chimera-ask-tab-submit-check');
      const allAnswered = this.questions.every((_, i) => this.isQuestionAnswered(i));
