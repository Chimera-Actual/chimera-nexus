...
export class TabBar {
  ...
  private build(): void {
    this.containerEl.addClass('chimera-tab-badges');
  }
  ...
  private renderBadge(item: TabBarItem): void {
    ...
    let stateClass = 'chimera-tab-badge-idle';
    if (item.isActive) {
      stateClass = 'chimera-tab-badge-active';
    } else if (item.needsAttention) {
      stateClass = 'chimera-tab-badge-attention';
    } else if (item.isStreaming) {
      stateClass = 'chimera-tab-badge-streaming';
    }

    const badgeEl = this.containerEl.createDiv({
      cls: `chimera-tab-badge ${stateClass}`,
  ...
  destroy(): void {
    this.containerEl.empty();
    this.containerEl.removeClass('chimera-tab-badges');
  }
}
