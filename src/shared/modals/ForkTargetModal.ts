...
class ForkTargetModal extends Modal {
  ...
  onOpen() {
    ...
    this.modalEl.addClass('chimera-fork-target-modal');

    const list = this.contentEl.createDiv({ cls: 'chimera-fork-target-list' });
  ...
  private createOption(container: HTMLElement, target: ForkTarget, label: string): void {
    const item = container.createDiv({ cls: 'chimera-fork-target-option', text: label });
