...
function createWikilink(
  ...
  link.className = 'chimera-file-link internal-link';
 ...
 * Handles both our custom .chimera-file-link and Obsidian's .internal-link.
 */
export function registerFileLinkHandler(
  ...
  component.registerDomEvent(container, 'click', (event: MouseEvent) => {
    ...
    const link = target.closest('.chimera-file-link, .internal-link') as HTMLAnchorElement;
  ...
  const walker = document.createTreeWalker(
    ...
    NodeFilter.SHOW_TEXT,
    {
      acceptNode(node) {
        ...
        if (parent.closest('pre, code, a, .chimera-file-link, .internal-link')) {
          return NodeFilter.FILTER_REJECT;
        }
